...
MUD_PROBABILITY = 0.0
MAZE_SYMMETRIC = True
NB_PIECES_OF_CHEESE = [x * 1 for x in range(6)]
...
# For every cheese value, we count the mean number of moves needed to finish the game
meanMovesPerNbCheese = {fileName:[] for fileName in PLAYERS_FILE_NAMES}
for density in NB_PIECES_OF_CHEESE :
    for fileName in PLAYERS_FILE_NAMES :
        meanMoves = 0.0
        for test in range(NB_TESTS) :
            #print(fileName + " -- NbCheese " + str(NbCheese)[:3] + " -- Test " + str(test + 1) + "/" + str(NB_TESTS))
            result = startRandomGame(MAZE_WIDTH, MAZE_HEIGHT, density, MUD_PROBABILITY, MAZE_SYMMETRIC, NB_PIECES_OF_CHEESE, PREPROCESSING_TIME, TURN_TIME, GAME_MODE, OUTPUT_DIRECTORY, TIMEOUT, fileName)
            meanMoves += result["player1"]["totalNbMoves"] / NB_TESTS
        meanMovesPerNbCheese[fileName].append(meanMoves)

# We plot the result
for fileName in PLAYERS_FILE_NAMES :
    plt.plot(NB_PIECES_OF_CHEESE, meanMovesPerNbCheese[fileName])
plt.xlabel("Number Piece Of Cheese")
