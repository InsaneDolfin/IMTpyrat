...
def chemin_one_cheese(mazeMap,playerLocation,piecesOfCheese):
    global chemin
    routage=parcours(mazeMap,playerLocation)                     #On créé la table de routage
    chemin=[piecesOfCheese[0]]                   #liste de toutes les cases à parcourir (on part de l'arrivé et on cherche les prédeceseurs successif)
    predecesseur=piecesOfCheese[0]
    while predecesseur!=playerLocation:                  #tant qu'on est pas au point de depart
        chemin.append(routage[predecesseur]) #on ajoute le predecesseur
        predecesseur=routage[predecesseur]              #on cherche le predecesseur du predecesseur
    chemin.pop()                           #on enleve la pos de depart
    return chemin
...
def preprocessing (mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed) :
    ...
    print("<b>[chemin le plus court]</b> " + str(chemin_one_cheese(mazeMap,playerLocation,piecesOfCheese)))
