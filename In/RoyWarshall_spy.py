# -*- coding: utf-8 -*-
"""
Created on Fri Sep 30 18:39:24 2016
...
def initialisation(mazeMap):
    distances=numpy.zeros(math.sqrt(mazeMap),math.sqrt(mazeMap))
    for i in int(math.sqrt(len(mazeMap))):
        for j in int(math.sqrt(len(mazeMap))):
            if i==j:
                distances[i][j]=0
            if i==mazeMap[j] or j==mazeMap[j]:
                distances[i][j]=mazeMap[i][j]
            else:
                distances[i][j]=float("inf")
    return distances
    
print(initialisation(mazeMap))
