...
"""
def ajouter_ou_remplacer(file_priorité, clé, valeur):
    file_priorité[clé]=valeur
    return file_priorité

def retirer(file_priorité):
    sommet=(0,0)
    distance=float("inf")
    for key in file_priorité:
        if file_priorité[key]<distance:
            sommet=key
            distance=file_priorité[key]
    file_priorité.pop(key)            
    return sommet,distance

def vide(file_priorité):
    if file_priorité=={}:
        return True
    else:
        return False
...
def chemin_court(mazeMap,playerLocation,piecesOfCheese,num_cheese):
    ...
    routage=dijkstra(mazeMap,playerLocation)                     #On créé la table de routage
